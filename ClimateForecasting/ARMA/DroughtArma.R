...
ggAcf(drought.ts.log) +
  ggtitle("Log ACF of California Droughts Time Series")

drought.ts.diff <- diff(diff(drought.ts.ca))
ggAcf(diff(drought.ts.ca)) +
  ggtitle("Log/Diff ACF of California Droughts Time Series") #q=1-2
pacf(diff(drought.ts.ca), main = 'PACF of California Floods Time Series') # p=1-7
...
fit <- Arima(drought.ts.ca, order=c(1,1,1), include.drift = FALSE)
png('../arma_visualizations/drought/drought_arima_forecast_1_1_1.png', width = 8, height = 6, units = 'in', res = 400)
autoplot(forecast(fit))
dev.off()

png('../arma_visualizations/drought/drought_sarima_forecast_1_1_1.png', width = 8, height = 6, units = 'in', res = 400)
sarima.for(drought.ts.ca, 60, 1,1,1)
dev.off()

viz.forecast <- drought.ts.ca %>%
  Arima(order=c(1,1,1),include.drift = FALSE) %>%
  forecast %>%
  autoplot() +
  ylab("Number of Droughts") + xlab("Year")
ggsave('../arma_visualizations/drought/drought_forecast_1_1_1.svg', plot = viz.forecast, width = 10, height = 6, units = 'in')

length(drought.ts.ca)

train = ts(drought.ts.ca[1:1200])
test = ts(drought.ts.ca[1201:1524])
fit <- Arima(train, order = c(1,1,1), include.drift = FALSE)
summary(fit)
