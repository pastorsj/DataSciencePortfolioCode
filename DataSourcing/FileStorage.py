...
class FileStorage:
    ...
    def store_as_processed_file(self, file_name, contents):
        """Stores a file in the processed data directory
        ...
        ----------
        """
        file_path = f'{self._processed_base_path}/{file_name}'
        self.create_directory_if_not_exists(file_path)
        file = open(file_path, 'w')
        file.write(contents)
        file.close()
    ...
    def store_processed_df_as_file(self, file_name, df):
        """Stores a dataframe as file in the processed data directory
